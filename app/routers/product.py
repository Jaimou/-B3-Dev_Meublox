...
def fetch_products(nom: Optional[str] = Query(None, alias="nom"),
    ...
    if sort is not None:
        ...
        if array[0] in sorting_keys:
            if array[1] == 'asc':
                products = list(db.get_collection("products").find(query).sort(array[0], pymongo.ASCENDING))
                return products
                
            else :
                products = list(db.get_collection("products").find(query).sort(array[0], pymongo.DESCENDING))
                return products
...
def fetch_products_by_id(id_prod: str, request: Request):
    if (product := db.get_collection("products").find_one({"_id": ObjectId(id_prod)})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with ID {id_prod} not found")
...
def delete_product(id: str, request: Request, response: Response):
    delete_result = request.app.database["products"].delete_one({"_id": ObjectId(id)})
...
def search_product_by_name(nom: str, request: Request):
    if (product := db.get_collection("products").find_one({"nom": nom})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with name {nom} not found")
...
def search_product_by_price(prix: float, request: Request):
    if (product := db.get_collection("products").find_one({"prix": prix})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with price {prix} not found")
...
def search_product_by_color(couleur: str, request: Request):
    if (product := db.get_collection("products").find_one({"couleur": couleur})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with color {couleur} not found")
...
def search_product_by_note(note: int, request: Request):
    if (product := db.get_collection("products").find_one({"note": note})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with note {note} not found")
...
def search_product_by_category(categorie: str, request: Request):
    if (product := db.get_collection("products").find_one({"categorie": categorie})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with category {categorie} not found")
...
def search_product_by_description(description: str, request: Request):
    if (product := db.get_collection("products").find_one({"description": description})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with description {description} not found")
...
def search_product_by_image(image: str, request: Request):
    if (product := db.get_collection("products").find_one({"image": image})) is not None:
        return product
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"product with image {image} not found")
